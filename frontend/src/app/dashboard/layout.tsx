"use client"
import { Shield, Activity, AlertTriangle, Search, Terminal, GitBranch, Settings, Bell, Users, Zap, LogOut } from "lucide-react"
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#0A0B0E] text-white">
      {/* Sidebar */}
      <aside className="w-14 flex flex-col items-center py-4 border-r border-[#1E293B] bg-[#0A0B0E] z-10">
        {/* Logo */}
            ...
            return (
              <Link key={href} href={href}
                className={`relative flex items-center justify-center w-full h-10 rounded-lg transition-all group ${
                  active ? "bg-cyan-500/15 text-cyan-400 shadow-[inset_0_0_0_1px_rgba(6,182,212,0.3)]" : "text-slate-600 hover:text-slate-300 hover:bg-[#1E293B]"
                }`}
                title={label}
              >
                <Icon size={17} />
        ...
        <header className="h-12 border-b border-[#1E293B] flex items-center px-4 justify-between bg-[#0A0B0E] flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-slate-500 font-mono uppercase tracking-wider">
              {NAV_ITEMS.find(n => pathname.startsWith(n.href))?.label || "SOC Platform"}
            </span>
          </div>
          <div className="flex items-center gap-3 text-xs font-mono">
            ...
            <span className="px-2 py-1 bg-red-500/10 border border-red-500/20 rounded text-red-400">CRITICAL: 3</span>
